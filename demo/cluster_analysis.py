...
def main(args: list[str]) -> None:
    ...
    if len(args) != 2:
        ...
        sys.exit(1) # Fix: changed demo/demo_cluster_analysis.py to demo/cluster_analysis.py. Isn't a major fix but isn't the correct path so could confuse user

    # Input CSV file - Fix: replace args[1] with args[0]
    input_path = args[1]
    ...
    if len(numeric_cols) < 2: # FIX: Changed from < 5 to < 2 for 2D clustering
